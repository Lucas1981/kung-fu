...
export default class Collision {
  ...
  private static animationFactory: AnimationFactory = AnimationFactory.getSingletonAnimationFactory();

  public static isColliding(source: Player, target: Player): boolean {
  ...
  private static squareBoxCollision(sourcePlayer: Player, targetPlayer: Player): boolean {

    let source = sourcePlayer.getCurrentBox();
    let target = targetPlayer.getCurrentBox();
    let canvas = Canvas.getSingletonCanvas();

    let sx1 = source.x;
    let sy1 = source.y;
    let sx2 = sx1 + source.width;
    let sy2 = sy1 + source.height;

    let dx1 = target.x;
    let dy1 = target.y;
    let dx2 = dx1 + target.width;
    let dy2 = dy1 + target.height;
